...
if (CLIENT) then
	...
	concommand.Add("+RGAssistant", function() 
		local self = ix.plugin.Get("acidcombine")
		local client = LocalPlayer()
		local char = client:GetCharacter()
		local inventory = char:GetInventory()
		...
		-- Function to extract the rank
		function extractRank(input)
			local rank = input:match(":(%w+)%.[A-Z]+%-")
			return rank
		end

		-- Function to extract the callsign
		function extractCallsign(input)
			local callsign = input:match(":(%w+)%-%d:")
			return callsign
		end

		-- Function to extract the ID
		function extractID(input)
			local id = input:match("%-(%d):%d+")
			return tonumber(id)
		end
		...
		local rank = extractRank(name) or "RANK"
		local tag = extractCallsign(name) or "TAG"
		local id = extractID(name) or 0
		...
		do
				...
				options = {
					...
					["I'm last man standing"] = function()
						...
						Radio(Pick({
							...
							"REQUEST RESERVE",
							"SECTOR; OUTBREAK; REQUEST RESERVE",
			...
			local contacts = self:AddAssistantOption(reports, "Contact", nil, {
				...
				options = {
					["Headcrab"] = function ()
						Radio(Pick({
							"OVERWATCH; SECTOR NOT STERILE;FREE PARASITE;SECTOR;" .. formattedPos,
					...
					["Zombies"] = function()
						Radio(Pick({
							"OVERWATCH; SECTOR NOT STERILE;NECROTICS;GRID;" .. formattedPos,
							"OVERWATCH; INFESTATION ZONE; GRID;" .. formattedPos,
							"OVERWATCH; INFESTATION ZONE; SECTOR;" .. formattedPos,
