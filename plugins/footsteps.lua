...
if CLIENT then
	...
	function GAMEMODE:GetDefaultStepSound(client, side)
		...
		elseif client:WaterLevel() >= 1 then
			...
			-- Begin Edit: AcidCloud, TheLife
			local factionTable = ix.faction.Get(client:Team())
			if (factionTable.runSounds and client:IsRunning()) then
				local fsnd = factionTable.runSounds[side and 1 or 0]
				if fsnd then snd = fsnd end
			end

			if (factionTable.walkSounds and !client:IsRunning()) then
				local fsnd = factionTable.walkSounds[side and 1 or 0]
				if fsnd then snd = fsnd end
			end

			-- End Edit: AcidCloud, TheLife
